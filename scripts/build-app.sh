...
#
# 서명은 ad-hoc(`--sign -`) 이다. 유료 인증서가 없어 "확인되지 않은 개발자" 표기가 남고,
# **다시 빌드할 때마다 위치 권한이 풀린다** (2026-07-29 실기 확인).
#   ad-hoc 서명에는 고정된 신원이 없어 TCC 가 코드 해시로 앱을 가른다. 코드가 한 글자만 바뀌어도
#   해시가 달라지므로 macOS 는 **이름만 같은 다른 앱**으로 보고 위치 권한을 다시 묻는다.
#   알림 권한은 번들 식별자에 매여 있어 남고, 전환 권한·설정은 앱 밖(/usr/local)에 있어 영향이 없다.
#   같은 이유로 로그인 항목을 켜 둔 채 앱을 바꾸면 '백그라운드에서 실행될 수 있습니다' 알림이 한 번 뜬다.
#
...
이미 쓰던 앱을 이 번들로 바꾸는 경우
  - 위치 권한이 풀립니다. ad-hoc 서명이라 코드가 바뀌면 macOS 가 다른 앱으로 봅니다 —
    설정 창의 권한 항목에서 [허용 요청] 을 한 번 누르면 됩니다
  - 로그인 항목을 켜 두었다면 '백그라운드에서 실행될 수 있습니다' 알림이 한 번 뜹니다 (무시해도 됩니다)
  - 전환 권한·설정 값·알림 권한은 그대로 남습니다
NEXT
