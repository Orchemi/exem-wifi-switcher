...
/// 자동 전환 on/off 를 담아 두는 곳.
///
/// 앱에서는 `UserDefaults`, 테스트에서는 메모리에 담는다. 설정 파일(`config.json`)에 두지 않는 이유는
/// 그 파일이 `root:admin` 소유라 저장할 때마다 권한이 필요하고, 이 값은 기기·사용자별 취향이기 때문이다.
public protocol AutoSwitchStore: AnyObject {
    func boolValue(forKey key: String) -> Bool?
    func setBool(_ value: Bool, forKey key: String)
}

extension UserDefaults: AutoSwitchStore {
...
public enum AutoSwitchPreferences {
    ...
    public static func isEnabled(in store: AutoSwitchStore) -> Bool {
        store.boolValue(forKey: enabledKey) ?? true
    }

    public static func setEnabled(_ enabled: Bool, in store: AutoSwitchStore) {
