...
| Wi-Fi 이름 칸 | 자유 편집 / 잠금 / 잠금+예외 | **잠금 + 예외 둘** (2026-07-28) | 자동으로 채워진 이름을 실수로 고치면 자동 전환이 조용히 걸리지 않는다. 다만 완전히 잠그면 **사내 Wi-Fi 가 여럿인 경우**(일반망·게스트망)와 **사외에서 설정하는 경우**가 막힌다. 그래서 ① 비어 있으면 잠그지 않고 ② 다른 사내 Wi-Fi 에 접속한 채로 열면 [지금 Wi-Fi 추가]로 더한다(손으로 적지 않아 오타가 없다) ③ 그래도 고쳐야 하면 [수정]으로 연다 |
