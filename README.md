...
<p align="center"><img src="docs/screenshots/menubar-menu.png" width="196" alt="메뉴바 아이콘을 눌러 펼친 메뉴. '사내 고정 IP' 프로필에 체크가 서 있고, '자동 전환' 이 켜져 있어 프로필 항목은 흐리게 보이는 상태. 그 아래에 설정과 종료." /><br><sub>설정을 마치면 평소에는 이 메뉴만 보입니다. 지금 무엇이 서 있는지는 체크 표시가 말해 줍니다.</sub></p>

## EXEM 사내 전용입니다

범용 Wi-Fi 전환 도구가 아닙니다. 상대하는 네트워크는 이름이 **`EXEM` 으로 시작하는 사내 Wi-Fi** 하나이고, 넣어야 하는 값(고정 IP·서브넷 마스크·라우터·사내 DNS)은 **사내에서만 얻을 수 있습니다.**
그래도 공개 저장소에 두는 이유는 이 도구가 `sudoers` 에 **암호 없이 root 로 실행되는 규칙**을 설치하기 때문입니다. 확인할 수 없는 것을 믿어 달라고 하지 않으려고 [무엇이 어디에 놓이는지](https://github.com/Orchemi/exem-wifi-switcher/wiki/what-gets-installed)와 [설치 버튼이 실제로 실행하는 스크립트](./scripts/install.sh)를 열어 두었습니다.
읽은 Wi-Fi 이름과 위치는 저장하거나 외부로 보내지 않습니다.
...
4. 설정 창의 **권한** 항목에서 **[설치]** 를 누릅니다. 무엇을 어디에 놓을지 먼저 보여주고, 확인하면 관리자 암호를 한 번 묻습니다
...
터미널로 격리 속성을 지우는 방법(`xattr -d com.apple.quarantine`)도 있지만 권하지 않습니다. 그 명령은 **모든 앱에
대해** 통하는 습관이 되기 쉽고, 정작 위험한 앱을 열 때도 손이 먼저 나가게 됩니다.

</details>

## 제거

설정 창의 **권한** 항목 옆 **[제거]** 를 누릅니다. 지울 목록을 먼저 보여주고 관리자 인증을 한 번 받습니다. 무엇을 되돌리고 **무엇이 남는지**는 [설치되는 항목 전부](https://github.com/Orchemi/exem-wifi-switcher/wiki/what-gets-installed) 에 있습니다.
...
`./scripts/uninstall.sh` 를 쓰면 됩니다. 지우는 대상은 앱 밖에 있습니다. 두 길은 같은 파일입니다.

터미널로 제거할 때는 **로그인 항목이 빠집니다.** 그것만은 파일이 아니라 macOS가 들고 있는 기록이라 셸에서 끌 방법이
없습니다. 스크립트가 그 자리에서 어떻게 끄는지 알려 줍니다. 앱의 **[제거]** 를 쓰면 앱이 함께 끕니다.
...
**창구는 이 저장소의 [Issues](https://github.com/Orchemi/exem-wifi-switcher/issues) 하나입니다** (메일 주소를 공개 저장소에 적지 않으려고 비공개 경로는 두지 않았습니다). Issues 는 공개되니 **사내 IP·라우터·DNS 주소·MAC 같은 값을 붙여넣지 마시고** 문서용 예약 대역(`192.0.2.x`)으로 바꿔 적어 주세요.
