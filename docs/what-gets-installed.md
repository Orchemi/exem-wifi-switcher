...
- **위치 권한 기록.** **시스템 설정 > 개인정보 보호 및 보안 > 위치 서비스** 에서
  `EXEM Wifi Switcher` 항목을 지웁니다. macOS는 이 기록도 명령으로 지우는 방법을 제공하지 않습니다.
  남겨 두시면 나중에 다시 설치하실 때 그 허용이 그대로 살아 있어 macOS가 다시 묻지 않습니다
